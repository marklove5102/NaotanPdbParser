...
#define DOWNLOAD_WEB "http://msdl.microsoft.com/download/symbols/"
	...
	inline PdbDownloader::PdbDownloader(const std::string &file_name) : __file_name(file_name)
	{
		...
		if (image_base != 0)
		{
			...
			if (!isFileExits(__download_path))
			{
				...
				std::cout << "Downloading PDB..." << std::endl;
				std::cout << "URL: " << __download_url << std::endl;
				std::cout << "Path: " << __download_path << std::endl;

				// 尝试从主服务器下载
				HRESULT downloadResult = URLDownloadToFileA(NULL, __download_url.c_str(), __download_path.c_str(), 0, NULL);

				// 如果失败，尝试官方微软符号服务器
				if (FAILED(downloadResult))
				{
					std::cout << "Failed to download from mirror, trying Microsoft server..." << std::endl;

					// 构建微软官方符号服务器URL
					std::string ms_url = "http://msdl.microsoft.com/download/symbols/" + __pdb_name + "/" + __file_md5.c_str() + "/" + __pdb_name;
					std::cout << "Microsoft URL: " << ms_url << std::endl;

					downloadResult = URLDownloadToFileA(NULL, ms_url.c_str(), __download_path.c_str(), 0, NULL);
				}
				...
				if (FAILED(downloadResult))
				{
					...
					case INET_E_RESOURCE_NOT_FOUND:
						errorMsg = "PDB file not found on server (404). The PDB might not be available.";
						break;
					case INET_E_DOWNLOAD_FAILURE:
						errorMsg = "Download failure. Check network connection.";
						break;
					case INET_E_CANNOT_CONNECT:
						errorMsg = "Cannot connect to server. Check internet connection.";
						break;
					default:
						char buf[256];
						sprintf_s(buf, "Download failed with HRESULT: 0x%08X", downloadResult);
						errorMsg = buf;
					}

					std::cout << "Error: " << errorMsg << std::endl;
					std::cout << "PDB GUID: " << __file_md5 << std::endl;
					std::cout << "PDB Name: " << __pdb_name << std::endl;
				...
				std::cout << "PDB downloaded successfully!" << std::endl;
			}
			// 已经有了 不用再次下载
		}
		else
